...
REPITER_BOTS = [u'mafia_bot@jabber.cz/JabberBot',u'mafbot@mafiozo.in/JabberBot']
MAFIA_REMOTE = {}
FROM_BOT_CLIENT = {}
REPITER_STAT = {'byte':0,'stanza':0,'in':0,'jids':{},'last':0}
...
def mafiozo_leave_handler(g, n, r, a, cl):
...
def mafiozo_send_iq(cl = None, botjid = None, body = None, id = None, jid = None, typ = None, nick = None, cityid=None):
    ...
    #    pass
    try:
        REPITER_STAT['byte']+=sys.getsizeof(iq)
        REPITER_STAT['stanza']+=1
        REPITER_LOG.append(iq)
        if len(REPITER_LOG)>25:
            REPITER_LOG=REPITER_LOG[-25:]
    except: pass
...
def hnd_mafiozo_start(t, s, p):
    ...
    if p.isdigit():
        ...
        if n in MF_GET_RES: 
            ...
            MAFIA_REMOTE[jid] = {'bot':MF_GET_RES[n]['jid'], 'private':private, 'spec':None, 'cl':s[3], 'last':''}
        ...
        try:
            ...
            if not typ or typ == 'public':
                for x in [c for c in MAFIA_REMOTE.keys() if MAFIA_REMOTE[c].get('bot')==fromjid]:
                    ...
                    if not 'last' in MAFIA_REMOTE[x].keys():
                        MAFIA_REMOTE[x]['last']=''
                    if body == MAFIA_REMOTE[x]['last']:
                        continue
                    MAFIA_REMOTE[x]['last']=body
            ...
            if jid in MAFIA_REMOTE.keys():
                ...
                if body == MAFIA_REMOTE[jid]['last']:
                        return
                MAFIA_REMOTE[jid]['last']=body
...
def mafiozo_message(r, t, s, p):
    ...
    if jid in MAFIA_REMOTE.keys():
        ...
        if jid in MAFIA_MEM.keys() and p in [u'1',u'#']:
            ...
            MAFIA_REMOTE[jid] = {'bot':botjid, 'private':s[1]+'/'+s[2], 'spec':0,'last':''}
            mafiozo_send_iq(s[3], MAFIA_REMOTE[jid]['bot'], join, str(), jid, 'public', nick)

def repiter_stat(t, s, p):
    rep,n = '', 0
    if p.lower()==u'станзы':
        for x in REPITER_LOG:
            n+=1
            try: rep+=str(n)+') '+x.toXml()+'\n'
            except: rep+=str(n)+') Unable\n'
        reply(t, s, (rep if not rep.isspace() else u'Пусто'))
        return
    reply(t, s, u'Принято станз: '+str(REPITER_STAT['in'])+u',\nПередано станз: '+str(REPITER_STAT['stanza'])+u',\nОбъем переданных данных: '+byteString(REPITER_STAT['byte'])+u',\nJID-s :'+'\n'.join(REPITER_STAT['jids'].keys()))
    
register_message_handler(mafiozo_message)
register_command_handler(repiter_stat, 'репстат', ['все','игры'], 0, 'Статистика игрового Mafia-репитера', 'репстат', ['репстат','репстат станзы'])
