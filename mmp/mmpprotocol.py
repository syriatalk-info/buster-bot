...
class MMPGetStatusChange(MMPBaseHandler):
    def __init__(self,protocol):
        super(MMPGetStatusChange,self).__init__(protocol)
        self.packet_class = MMPChangeUserStatus
        self.auto_remove_handler = False
        #self.seq = seq
    ...
    def handlePacket(self,packet):
        self.protocol.callback.changeStatus(packet)

class MMPGetContactList(MMPBaseHandler):
    def __init__(self,protocol):
        super(MMPGetContactList,self).__init__(protocol)
        self.packet_class = MMPServerContactListPacket
        self.auto_remove_handler = False
    def canHandlePacket(self,packet):
        return isinstance(packet,self.packet_class)

    def handlePacket(self,packet):
        self.protocol.callback.contactList(packet)
...
class MMPProtocol(protocol.Protocol,MMPDispatcherMixin):
    ...
    def __init__(self,callback):
        ...
        self.addHandler(MMPGetContactList(self))
        self.addHandler(MMPGetStatusChange(self))
