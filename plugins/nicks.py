...
def hnd_nicks_inf(t, s, p):
        ...
        for x in NICKS[s[1]]:
                if p in NICKS[s[1]][x]['n']:
                        n+=1
                        rep+=u'Информаци о '+p+u':\nНики - '+', '.join(NICKS[s[1]][x]['n'].keys())+u'\nПервый раз увидел его в '+datetime.datetime.fromtimestamp(NICKS[s[1]][x]['t']).strftime('%Y-%m-%d')
                        rep+=u'\nСообщений - '+str(NICKS[s[1]][x]['m'])+u'\nПоследний раз видел в '+datetime.datetime.fromtimestamp(NICKS[s[1]][x]['t']).strftime('%m-%d %H:%M:%S')+'\n'
