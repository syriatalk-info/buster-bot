...
def hnd_uptime_top(t, s, p):
    ...
    rep=str()
    for x in d:
        rep+=str(d.index(x)+1)+')'+x[0]+' - '+stod(x[1])+'\n'
        if d.index(x)>30:
            break
...
register_command_handler(hnd_uptime_top, 'выбрать сервер', ['админ','все'], 0, 'Топ-30 jabber-серверов на основе времени работы.', 'выбрать сервер', ['выбрать сервер'])
