...
def get_participant_answ(g, cljid, x):
    ...
    list=[u'jabber.ru',u'talkonaut.com',u'xmpp.ru',u'jabber.ua',u'qip.ru',u'jabberon.ru']
...
def security_set(type, source, parameters):
    ...
    if parameters.lower()==u'auto':
        SECURITY[source[1]]={'time':3, 'n':2}
        write_file(SECURITY_FILE,str(SECURITY))
        reply(type, source, u'Автонастройка завершена!')
        return
    f = ''
    jid = get_true_jid(source[1]+'/'+source[2])
    if source[1] in SECURITY.keys():
        f+=u'Активные настройки: включен лимит на более '+str(SECURITY[source[1]].get('n',1))+u' новичков за период '+str(SECURITY[source[1]].get('time',1))+u'с.'
        
    SECURITY_MSG[jid]={}
    SECURITY_MSG[jid]['step1']=time.time()
    reply(type, source, u'Вы в меню настройки безопасности конференции, у вас 2 минуты на таймере, выберите дейстивие:\n1-включить/настроить лимит новых пользователей,\n0-отключить\n'+f)
...
def security_join(g, n, r, a, cljid):
    ...
    try:
        if not g in GROUPCHATS or not  get_bot_nick(g) in GROUPCHATS[g]:
            return
        ...
        if time.time()-BOTS_GET_SELF_PRESENCE[g][n]<7:
            return
        if 'NICKS' in globals().keys() and g in NICKS.keys() and jid in NICKS[g].keys() and time.time()-NICKS[g][jid]['t']>600:
            return#плагин nicks.py проверка заходил ли гость раньше
    except: pass
...
def security_msg(raw, type, source, parameters):
    ...
    if 'step1' in SECURITY_MSG[jid] and time.time()-SECURITY_MSG[jid]['step1']<120:
        if int(parameters)==1:
...
register_command_handler(security_set, 'security', ['все','антивайп'], 20, 'Настройка лимита новых пользователей в конференции.\nКлюч auto включит бан при входе более двух гостей за промежуток короче 3-х с.', 'security', ['security'])		
