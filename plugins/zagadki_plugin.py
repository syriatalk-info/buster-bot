...
#                for i in xrange(len(result))][:10]))
            msg(s[3], groupchat, u'Текущий счёт этой конференции'+ u':\n' + \
                u'\n'.join([u"%d) %s: %d баллов" % (i+1,  result[i][1], result[i][0]) \
                for i in xrange(len(result))][:10]))
...
####            msg(groupchat, u'%s занимает %d место! Заработав %d очков' %\
####                (your_points, place))
#            msg(groupchat, u'Текущий счёт '+groupchat+ u':\n' + \
#                u'\n'.join([u"%d) %s: %d баллов" % (i+1,  result[i][1], result[i][0]) \
...
def handler_zagadki_otvet(type, source, parameters):
    ...
    if not groupchat:
        ...
        return
    if ZAGADKI_CURRENT_ANSWER.has_key(groupchat):
        msg(groupchat, u'Правильный ответ: ' + ZAGADKI_CURRENT_ANSWER[groupchat])
    else:
        reply(type, source, u'Игра в загадки не запущена, для запуска напишите .вкл')
...
def handler_zagadki_me_scores(type, source, parameters):
    ...
    zagadki_me_list_scores(groupchat)
...
register_command_handler(handler_zagadki_hint, '.', ['загадки','развлечения','все'], 0, ' Показывает подсказку', '.', ['.'])
