...
def invite_to_bot(r, t, s, p):
    ...
    if r['type']=='normal':
        ...
        if arab:
            reply(t, s, u'russian only!')
            return
        
        if not jid in INV_TOB_LIM:
            INV_TOB_LIM[jid]=time.time()
        else:
            if time.time()-INV_TOB_LIM[jid]<86400:
                reply(t, s, u'В целях безопасности лимит один инвайт в сутки!')
                return
        INV_TOB_LIM[jid] = time.time()
        if chat in GROUPCHATS.keys():
            reply(t, s, u'Я уже там!')
            return
        if len(INV_TOB_LIM)>10:
            reply(t, s, u'За рабочую сессию бот получил более 10-ти инвайтов, в целях безопасноти повторный ивайт может быть обработан только после рестарта бота!')
            return
