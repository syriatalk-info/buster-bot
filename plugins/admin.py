...
def hnd_join_result(type, source, parameters, x):
    ...
    db[source[3]][parameters]={'from':frm, 'nick':DEFAULT_NICK, 'status':DEFAUL_STATUS, 'show':'chat'}
    write_file(file, str(db))
...
def hnd_remote_cmd(t, s, p):
    ...
    if not cmd in COMMANDS:
        ...
        return
    par=' '.join(ss[2:])
    try: ajid=random.choice([x for x in GLOBACCESS if GLOBACCESS[x]==100])
    except:
        reply(t, s, u'В настройках бота нет ботадмина либо произошла ошибка!')
        return
    try: bjid = GROUPCHATS[chat][get_bot_nick(s[1])]['jid']
    except: return
    ba=u'botadmin'
