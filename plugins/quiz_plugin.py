...
try: FNWQYI = eval(read_file('dynamic/newcmquiz.txt'))
except: FNWQYI = {}

def hnd_nwqyi(t, s, p):
        global FNWQYI
        jid = get_true_jid(s)
        if not jid in FNWQYI:
                FNWQYI[jid]={}
                reply(t, s, u'команда переименована в !игра')
                write_file('dynamic/newcmquiz.txt', str(FNWQYI))
