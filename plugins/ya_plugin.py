...
def ya_week(t, s, p):
    ...
    rep = rep.replace('    ',' ').replace('хмарність','облачность').replace('дощ','дождь').replace('хмарно','облачно').replace('снiг','снег').replace('мінлива','переменная').replace('з проясненнями','с прояснениями')
...
register_command_handler(hnd_autogr, 'autogr', ['все'], 0, 'Сервис проверки правописания Яндекс.Спеллер для конференций.', 'autogr', ['autogr'])
