...
def hnd_getold_list(type, source):
        if check_file(source[1],'banlist.txt'):
                ...
                n=0
                if txt:
                        hnd_banl_packet(source, 'outcast', txt.keys())
...
def any_copy_banl(type,source,parameters):
        ...
        if txt:
                hnd_banl_packet(source, 'outcast', txt.keys())
        reply(type,source,u'Bсего скопировано банов: '+str(len(txt)))
...
def old_ban(cljid, groupchat, jid, reason):
...
def hnd_now_unban(cljid, groupchat, jid):
        q = domish.Element(('jabber:client', 'iq'))
        q['type'] = 'set'
        q['id'] = str(random.randrange(1,999))
        q['to'] = groupchat
