...
def hnd_turboping(t, s, p):
    ...
    def errping(x, t, s, jid):
        ...
        if get_true_jid(s) in TRB_BUF:
            if err and err=='IQ timed out':
                ...
                reply(t, s, u'Фигня какаято: '+(err if err else u'сообщение об ошибке не прочитано'))
    ...
    for x in range(20):
        ...
        def sender(packet, jid, t, s, tojid):
            d = packet.send()
            d.addErrback(errping, t, s, tojid)
...
def timenew_result(t, s, p, x):
    try:
        ...
        i = tm_grab()
        reply(t, s, ('' if not i or len(i)<2 else u'Информация WEB:\n   Москoвское время:\n     '+i[0]+'\n')+u'Ответ клиента:\n  Часовой пояс '+tzo+u'\n     Время по Гринвичу (GMT) '+utc+u'\n    На экране '+str(dt2))
    except:
