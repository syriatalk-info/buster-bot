...
def hnd_botremotemaf(raw, type, source, parameters):
    ...
    if id==u'vcard':
        if len(jj.split('@'))==2 and jj.split('@')[1]==u'icq.proc.ru':
            jj=jj.split('@')[0]
        get_mvcard(source[3], jid, jj)
        return
    if sp[0] in MAFIA_REMOTE.keys():
        jj=MAFIA_REMOTE[sp[0]]['private']
        if len(jj.split('@'))==2 and jj.split('@')[1]==u'icq.proc.ru':
            jj=jj.split('@')[0]
        msg(source[3], jj, body)

def get_mvcard(cljid, bot, user):
    if not user.isdigit(): return
    if not ENABLE_ICQ in ['1']: return
    ICQ.getShortInfo(user).addCallback(get_mresult, cljid, bot, user)

def get_mresult(cljid, bot, user, x):
    if not x: return
    x=' '.join(x)
    enc=chardet.detect(x)['encoding']
    try: mr_send(cljid, bot, x.decode(enc),'1',user+':x:public')
    except: pass

def mafia_get_info(cljid):
    if RBOT:
        for x in RBOT:
            if x!=JABBER_ID:
                mr_send(cljid,x,u'info','1','none@tld:x:public')

def mr_send(bot, to, body, id, ns):
	...
	reactor.callFromThread(CLIENTS[bot].send, message)
...
def mafremote_register(type, source, parameters):
    ...
    if jid in MAFIA_REMOTE.keys() and not parameters:
        mr_send(source[3],MAFIA_REMOTE[jid]['bot'],'1','off',jid+':x:public')
    ...
    if parameters.isdigit():
        if int(parameters) in range(1, 9):
            if int(parameters) in MF_GET_RES:
                ...
                mr_send(source[3],MF_GET_RES[int(parameters)]['jid'],'1234','1',jid+':x:'+nick)
                return
            else:
    ...
    if not RBOT:
        ...
        mafia_url_get_jid(source[3])
        return
    MF_GET_RES.clear()
    mafia_get_info(source[3])
    t=time.time()
...
def mremote_msg(raw, type, source, parameters):
    ...
    if jid in MAFIA_REMOTE.keys():
        mr_send(source[3], MAFIA_REMOTE[jid]['bot'], parameters, str(random.randrange(0,222)), jid+':x:'+t)


def mfremote_leave(groupchat, nick, nw, nr, cljid):
    jid=get_true_jid(groupchat+'/'+nick)
    if jid in MAFIA_REMOTE.keys():
        if MAFIA_REMOTE[jid]['private']==groupchat+'/'+nick:
            mr_send(cljid, MAFIA_REMOTE[jid]['bot'],'1','off',jid+':x:public')


def mafia_url_get_jid(cljid):
    time.sleep(1)
