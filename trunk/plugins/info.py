...
register_command_handler(hnd_port_tel, 'port', ['все'], 0, 'Проверяет порт на подключение', 'port adres port', ['port mafiozo.in 5222'])
...
def stats_handler(t, s, p):
    ...
    reactor.callFromThread(packet.send, p, CLIENTS[s[3]])
