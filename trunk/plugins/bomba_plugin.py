...
def bomb_send(type, source, parameters):
        ...
        msg(source[3], source[1], u'/me '+parameters+e+l+u' провода: '+d+u' выберите цвет провода который нужно перерезать, на таймере '+unicode(tim)+u' sek.')
        PROVOD.remove(s)
        del FLL[parameters]
        print 'ok'
        bomb_start(source[3],source[1],parameters,tim)
...
def bomb_msg(raw,type,source,parameters):
        ...
        if source[2] in FLL2:
                ...
                if ad in FLL2[source[2]]['answ']:
                        msg(source[3], source[1], u'/me '+source[2]+u', бомба обезврежена!')
                        del FLL2[source[2]]
                        return
                else:
                        kick_bomb(source[3], source[1],source[2],u'птыдыщь!')
                        rep=random.choice(BOMB_FRA)
                        msg(source[3],source[1],u'/me '+source[2]+rep)
                        del FLL2[source[2]]
