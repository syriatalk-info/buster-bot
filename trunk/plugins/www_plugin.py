...
def hnd_www(type, source, parameters):
    ...
    if parameters.count('localhost') or parameters.count('file://') or parameters.count('ftp://') or parameters.count('///'):
    ...
    try:
        ...
        try: req = urllib2.urlopen(req, timeout = 3)
            ...
            return
        if req.geturl() != url:
            reply(type, source, u'Перенаправление запрещено! Ресурс перенаправляет на '+req.geturl())
            return
        data = req.read()
