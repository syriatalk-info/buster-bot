...
filtd = {'prs':u'Фильтр презенсов','nick':u'Фильтр частой смены никa','caps':u'Фильтр по наличию капса в презенсе','stmsg':u'Фильтр длинных статусных сообщений','like':u'Фильтр одинаковых сообщений','len':u'Фильтр длинных сообщений','speed':u'Фильтр скорости отправки сообщений','traf':u'Фильтр траффика'}
...
def hnd_filter_prs(prs, cljid):
    ...
    if not jid in FILTER_USER[groupchat].keys():
        FILTER_USER[groupchat][jid]={'traf':0, 'traftimer':0, 'prsl':0, 'lastprs':0, 'lastmsg':'','lastmsgtime':0,'nick':0,'lastnick':0}
    
    stmsg, status, afl, role, code, reason, caps = None, None, None, None, None, None, None
    try:
...
def hnd_filt_con(t, s, p):
    ...
    d = {'prs':u'Фильтр презенсов','nick':u'Фильтр частой смены никa','caps':u'Фильтр по наличию капса в презенсе','stmsg':u'Фильтр длинных статусных сообщений','like':u'Фильтр одинаковых сообщений','len':u'Фильтр длинных сообщений','speed':u'Фильтр скорости отправки сообщений','traf':u'Фильтр траффика'}
    if p in ['off','0']:
    ...
    if p.count(' '):
        ...
        if not sp[0].lower() in d.keys():
            reply(t, s, u'Неправильный синтаксис!')
            return
        if sp[1] in ['0','1']:
...
register_command_handler(hnd_filt_con, 'filt', ['все'], 20, 'Фильтр чата. Ключи: '+', '.join([x+' - '+filtd[x] for x in filtd.keys()]).encode('utf8'), 'filt <prs|stmsg|nick|len|caps|like|speed|traf|0|off> <0|1>', ['filt caps 1','filt len mode ban','filt','filt 0'])               
