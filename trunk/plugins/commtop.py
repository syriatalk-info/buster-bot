...
def msg_commtop(r, t, s, p):
    ...
    cmd = ss[0].lower()
    cmd = cmd.strip()
    cmd2 = ('' if len(ss)<2 else ' '.join(ss[:-2]).lower())
    if not cmd2.isspace() and cmd2 in COMMANDS:
        cmd = cmd2
    if cmd in COMMANDS:
        COMMTOP_CALC += 1
        if cmd in COMMTOP:
...
def hnd_commlast(t, s, p):
    rep = ''
    
    sor = sorted(COMM_LOG.items())
    sor.reverse()
    for x in sor:
        v = datetime.datetime.fromtimestamp(x[0])
        rep+= v.strftime('%H:%M:%S')+' '+COMM_LOG[x[0]]+'\n'
    reply(t, s, rep)


register_command_handler(hnd_commlast, 'комласт', ['все'], 100, 'Выводит список последних команд выполненных ботом', 'комласт', ['комласт'])
