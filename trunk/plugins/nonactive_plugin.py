...
def nonactive_msg(r, t, s, p):
        ...
        if not s[1] in NONACTIVE.keys():
                ...
                write_file(NONACT_FL, str(NONACTIVE))
...
def nonactive_init(*agrw):
        global NONACTIVE
        
        try:
                db = eval(read_file(NONACT_FL))
        except:
                write_file(NONACT_FL, '{}')
                db = dict()
...
def nonactive_load(*agrw):
        if not agrw[0] in NONACTIVE.keys():
                ...
                write_file('dynamic/nonactive.txt', str(NONACTIVE))
	
register_message_handler(nonactive_msg)
register_stage1_init(nonactive_load)
register_stage0_init(nonactive_init)
