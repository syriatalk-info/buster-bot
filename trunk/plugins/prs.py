...
def s2s_result_handler(t, gch, cljid, x):
    if x['type'] == 'error':
        if time.time()-INFO['start']<60 or time.time()-CLIENTS_UPTIME.get(cljid, 0)<60:
            pass
        else:
...
def check_jids_connect(*an):
    ...
    def resping(x, cljid, i):
        ...
        if x['type'] == 'result':
            ...
            if cljid in EMERG_BUF.keys():
                try:
                    for ch in GROUPCHATS:
                        if cljid in db.keys() and not ch in db[cljid]:
                            leave(ch, u'General JID now start working', EMERG_BUF[cljid])
                except: pass
    ...
    for x in BPING_STAT.keys():
        if BPING_STAT[x]['hist']:
            ...
            for c in BPING_STAT[x]['hist']:
                rep+='Day data  '+c+' ping sec. = '+BPING_STAT[x]['hist'][c]+'\n'
