...
def handler_help_help(type, source, parameters):
	...
	if parameters and COMMANDS.has_key(parameters.strip()):
		...
		rep = u'напиши слово "команды" (без кавычек), чтобы получить список команд, "помощь <команда>" для получения помощи по команде.\nЕсли возникают другие вопросы - связаться с админом бота можно через команду мессага_админу.'
