...
def chat_disco_info(typ, ss, p):
        ...
        for x in GLSRCINFO.keys():
                if GLSRCINFO[x].lower().count(p):
                        inf = GLSRCINFO[x]
                        if len(inf)>50:
                                inf = inf[:50]+'...'
                        rep+=x+' - '+inf+'\n'
...
def hnd_cmd_guide(t, s, p):
        ...
        i.sort()
        for x in i:
                try: a = COMMANDS[x]['desc'].decode("utf-8")
                except: continue
                if len(a)>80:
                        a = a[:80]+'...'
                rep+= x+' - '+a+'\n'
...
def handler_help_help(type, source, parameters):
	...
	rep = str()
	if not parameters:
                rep = u'Я бот - Бастер(болван)\n'
                rep += u'созданный для игры <<Мафия>>\n'
                rep += u'а также с поддержкой многих пользовательских функций для ростера и чата.\n'
                rep += u'Узнать мои команды можно написав <команды все>\n глянуть полный список команд с кратким описанием <комгид>\n либо найти команду по совпадению в описании <компоиск слово>.\n'
                rep += u'Для работы бот использует twisted framework, написан на Python.'
