...
TV2_HISTORY = 'dynamic/his_tv2.txt'
db_file(TV2_HISTORY, dict)
...
def msg_tv2(r, t, s, p):
    ...
    if jid in GLOBAL_TV2.keys():
        if p in GLOBAL_TV2[jid] and p!='t' and time.time()-GLOBAL_TV2[jid].get('t',0)<180:
...
def hnd_tv2(t, s, p, sp=0):
    ...
    if (list and len(list)==1) or sp or not list:
        ...
        try:
            ...
            day = day[len(day)-1]
            if not sp: chan = re.findall('/kanal(\d{1,10}_\d{8}).html', page, re.DOTALL | re.IGNORECASE)
        except: pass
        
        
        try: #page = '\n'.join(re.findall('(\d{2}:\d{2}.*?(</b>|</a>))',page,re.DOTALL|re.IGNORECASE)[1:])#</div>
            page = '\n'.join([re.split('</b>|</a>',x)[0] for x in re.findall('(\d{2}:\d{2}.*?)</div>',page,re.DOTALL|re.IGNORECASE)][1:])
        except: return
        #print chan
        page = title+'-'+day+'\n'+page
        if chan:
            jid = get_true_jid(s)
            try:
                GLOBAL_TV2[jid]={'-':chan[6],'+':chan[8],'t':time.time()}
                page+=u'\nПиши \"-\",\"+\" для перемещения по дате (таймер 3 минуты)'
            except: pass
        reply(t, s, dehtml(page))
        return
    rep=''
