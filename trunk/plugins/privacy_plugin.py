...
def check_iq_ddos(iq, cljid):
    ...
    try:
        uri = iq.firstChildElement().uri
        if uri in ['xmpp:iq:mafia','http://jabber.org/protocol/ibb','xmpp:iq:b2b']: return
    except: pass
    try:
    ...
    if fr in GROUPCHATS:
        return # Make fix this!!! Create limit for room muc-filter
    if not fr in DDOS_IQ.keys():
        DDOS_IQ[fr]={'last':time.time(),'n':0,'all':1}
    else:
        if time.time()-DDOS_IQ[fr]['last']<tt or sys.getsizeof(iq.toXml())>1024:
            DDOS_IQ[fr]['n']+=1
        else:
            DDOS_IQ[fr]['n'] = 0
        ...
        if DDOS_IQ[fr]['n']>=limit:
            ...
            time.sleep(1)
            privacy_iq(cljid, 'active', 'ignore')
            ddos_warning_send(cljid, u'В связи с подозрением на IQ-DDoS в боте активирован 2-х минутный игнор Jabber ID источника нагрузки!(смотри privacy)')
            time.sleep(120)
            try: PRIVACY_DENY.remove(fr)
            except: pass
            privacy_set(cljid)
