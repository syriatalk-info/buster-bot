                ...
                if time.time() - PRIVACY_NICKCHANGE[g]['t']<1:
                    if PRIVACY_NICKCHANGE[g]['ljid']!=get_true_jid(g+'/'+n):
                        ...
                        if PRIVACY_NICKCHANGE[g]['n']>40:
                            ...
                            msg(cljid, g, u'/me Чат блокирован, причина - подозрение на вайп.')
...
def check_iq_ddos(iq, cljid):
    ...
    if fr.split('/')[0]==cljid: return
    if not fr in DDOS_IQ.keys():
        DDOS_IQ[fr]={'last':time.time(),'n':0,'all':1}
    else:
        if time.time()-DDOS_IQ[fr]['last']<1 or sys.getsizeof(iq)>1024:
            DDOS_IQ[fr]['n']+=1
        else:
            DDOS_IQ[fr]['n'] = 0
