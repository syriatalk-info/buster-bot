...
def search_in_plugincode(t, s, p):
    ...
    for x in list:
        try: f=read_file(os.path.join('plugins',x)).splitlines()
        except: continue
        for c in f:
            if c.count(p):
                n+=1
...
register_command_handler(hnd_orab, 'ораб', ['все'], 100, 'specific command', 'ораб <room id>', ['ораб 11'])	
    ...
    for x in db.keys():
        for c in db[x].keys():
            if x!= source[3] and c == parameters:
                if level<40:
                    reply(type, source, u'Бот ранее был либо находится в комнате '+parameters+u' с другой учтеной записи! Для вывода бота нужен доступ не менее 40!')
                    return
                else:
                    reply(type, source, u'Бот ранее был либо находится в комнате '+parameters+u' с учтеной записи '+x+u'! Сейчас будет произведена смена конфигураций, подождите пару секунд!')
                        ...
                        for c in x.keys():
                            if c==parameters:
                                ...
                                if c in GROUPCHATS.keys():
                                    del GROUPCHATS[c]
                    time.sleep(2)
    frm = source[1][:3]+'.../'+source[2]
    db[source[3]][parameters]={'nick':DEFAULT_NICK, 'status':'I\'am from '+frm, 'show':'chat'}
    write_file(file, str(db))
...
def hnd_botup(type, source, parameters):
    rep=u'Время от запуска бота '+timeElapsed(time.time()-INFO['start'])+'\n'
    if "CLIENTS_UPTIME" in globals().keys():
        rep+=u'Активные подключения в Jabber:\n'
        for x in CLIENTS_UPTIME.keys():
            rep+='  '+x+u' - '+timeElapsed(time.time()-CLIENTS_UPTIME[x])+'\n'
...
def hnd_remote_cmd(t, s, p):
    ...
    call_command_handlers(cmd, 'private', [chat+'/'+ba, chat, ba, s[3]], par)
