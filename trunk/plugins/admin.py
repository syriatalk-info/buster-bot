...
def invite_to_bot(r, t, s, p):
    global INV_TOB_LIM
    #global zz
    try:
        if not GENERAL_CONFIG('ENABLE_INVITE_JOIN'):
            return
    except: pass
    jid, chat = None, None
    if r['type']=='normal':
        el = [x for x in r.elements() if x.uri=='http://jabber.org/protocol/muc#user']
        if not el: return
        inv = [x for x in el[0].children if x.name=='invite']
        if not inv: return
        jid = inv[0]['from']
        chat = r['from']

        try:
            zh(s[3], u'Инвайт от '+jid+u' в '+chat)
        except:
            pass

        sc = [jid, jid, '', s[3]]
        
        arab = [x for x in chat if unicodedata.category(x) in ['Lo']]
        if arab:
            reply('private', sc, u'russian only!')
            return
        
        if not jid in INV_TOB_LIM:
            INV_TOB_LIM[jid]=time.time()
        else:
            if time.time()-INV_TOB_LIM[jid]<3600:
                reply('private', sc, u'В целях безопасности лимит один инвайт в Час!')
                return
        INV_TOB_LIM[jid] = time.time()
        if chat in GROUPCHATS.keys():
            reply('private', sc, u'Я уже там!')
            return
        if len(INV_TOB_LIM)>10:
            reply('private', sc, u'За рабочую сессию бот получил более 10-ти инвайтов, в целях безопасноти повторный ивайт может быть обработан только после рестарта бота!')
            return
...
register_message_handler(invite_to_bot)
...
def turn_byte(b):
    ...
    i = (b/1024)/1024.0
    if i> 1:
        return str(round(i, 1))+'M'
    return str(round(i, 3))+'M'
...
def traf_iq_muc(iq, cl):
    ...
    TRAF_FROM_MUC[fromjid.split('/')[0]]+=sys.getsizeof(iq.toXml())
...
def traf_msg_muc(r, t, s, p):
    ...
    TRAF_FROM_MUC[s[1]]+=sys.getsizeof(r.toXml())
...
def traf_prs_muc(prs, cl):
    ...
    TRAF_FROM_MUC[fromjid.split('/')[0]]+=sys.getsizeof(prs.toXml())
...
def hnd_wherei(type, source, parameters):
    global TRAF_FROM_MUC
    tup = []
    rep, n, inmuc, jid = '', 0, 0, 0
    ...
    for x in txt.keys():
        ...
        rep+='JID ['+str(jid)+'] - '+x+'\n'
        for g in txt[x].keys():
            k[g]=TRAF_FROM_MUC.get(g,0)
        tup = sorted(k.items(), key=operator.itemgetter(1))
        tup.reverse()
        for c in tup:#txt[x].keys():
            c = c[0]
...
def hnd_botup(type, source, parameters):
    ...
    rep+=u'- Траффик In '+byteString(INFO['tin'])+u' Out '+byteString(INFO['tout'])+u'\n'
    rep+=u'- Отправлено сообщений всего '+str(INFO['out'])+'\n'
    rep+=u'- Запросов на авторизацию '+str(INFO['auth'])+'\n'
    try: rep+=u'- Нагрузка станз в секунду '+str(round(INFO['all']/(time.time()-INFO['start']),2))
    except: pass
    reply(type, source, rep)
...
def hnd_ban(type, source, parameters):
    ...
    if parameters:
        ...
        if parameters in nicks:
            moderate_set(type, source, parameters, 'affiliation', 'outcast', 'jid', get_true_jid(GROUPCHATS[source[1]][parameters]['jid']), source[2])
        else:
            if parameters.count('.'):
                moderate_set(type, source, parameters, 'affiliation', 'outcast', 'jid', parameters, source[2])
    else:
...
def hnd_clear(type, source, parameters):
    ...
    if not source[1] in GROUPCHATS:
        return
    if not source[1] in CLEAR_VRT.keys():
        CLEAR_VRT[source[1]]=time.time()
    else:
        if time.time()-CLEAR_VRT[source[1]]<60:
...
def hnd_remote_report(jid, body, any):
    ...
    if jid in REMOTE_REPORT.keys():
        if REMOTE_REPORT[jid][0]!=None:
            reply(REMOTE_REPORT[jid][0],REMOTE_REPORT[jid][1],body)
            for x in GROUPCHATS:
                if REMOTE_REPORT[jid][2] in GROUPCHATS[x].keys():
                    try:
                        del GROUPCHATS[x][REMOTE_REPORT[jid][2]]
                    except:
                        pass
    #print jid, body
...
def hnd_global_remote(t, s, p):
    ss = p.split()
    cmd = ss[0].lower()
    if not cmd in COMMANDS:
        reply(t, s, u'Нет такой команды!')
        return
    par = ('' if len(ss)==1 else ' '.join(ss[1:]))
...
def hnd_remote_cmd(t, s, p):
    ...
    chat = ss[0].lower()
    if chat.lower()==u'везде':
        chat = GROUPCHATS.keys()
    else:
        if not chat in GROUPCHATS:
            list = [m for m in GROUPCHATS.keys() if m.count(chat)]
            if list:
                if len(list)>3:
                    reply(t, s, u'Укажите чат более детально!(>3 совпадений)')
                    return
                chat = list
            else:
    ...
    tt = t
    if len(chat)>1 and len(chat)==len(GROUPCHATS.keys()):
        tt=None
    REMOTE_REPORT[ajid] = (tt, s, ba)
    GLOBACCESS[ajid] = 100
    
    for x in chat:
        GROUPCHATS[x][ba]={'stmsg':'','status':'','ishere':1,'jid':ajid,'joined':time.time(),'idle':time.time()}

    for x in chat:

        try:
            bjid = GROUPCHATS[x][get_bot_nick(x)]['jid'].split('/')[0]
            if not bjid in CLIENTS:
                raise error('Any')
        except:
            d = eval(read_file('dynamic/chatroom.list'))
            bjid = [c for c in d.keys() if x in d[c].keys() and c in CLIENTS]
            if bjid:
                bjid=bjid[0]
            else:
                reply(t, s, u'Упс, что-то не получилось выполнить в '+x)
                continue
        ...
        #ajid = ba+'@uknown'+id+'.tld'
        #tt = t
        #if len(chat)>1 and len(chat)==len(GROUPCHATS):
        #    tt=None
        #REMOTE_REPORT[ajid] = (tt, s)
        #GLOBACCESS[ajid] = 100
        #GROUPCHATS[x][ba]={'stmsg':'','status':'','ishere':1,'jid':ajid,'joined':time.time(),'idle':time.time()}
        
        #call_command_handlers(cmd, 'private', [x+'/'+ba, x, ba, bjid], par)
        try: threading.Thread(None,call_command_handlers,'remote_cmd'+str(INFO['thr']),(cmd, 'private', [x+'/'+ba, x, ba, bjid], par,)).start()
        except: pass
        time.sleep(1.5)
...
register_command_handler(hnd_remote_cmd, 'ремоут', ['все'], 100, 'Дистанционное выполнение команды в любой из конференций где находится бот.', 'ремоут <чат> <команда> <параметры>', ['ремоут везде сказать посоны привет','ремоут cool избани gogi.net'])        
