    ...
    if x['type'] == 'error':
        ...
        if code:
            ...
            if code[0] in ['403']:
                ...
                return
            reply(type, source, u'Код ошибки '+code[0])
            return
...
def hnd_botup(type, source, parameters):
    ...
    if "CLIENTS_UPTIME" in globals().keys():
        ...
        for x in CLIENTS_UPTIME.keys():
            rep+='  JID: '+x+u' - '+(timeElapsed(time.time()-CLIENTS_UPTIME[x]) if hasattr(CLIENTS.get(x),'send') else u'(Отключен)')+'\n'
        if icq:
            rep+='  UIN: '+UIN+'\n'
