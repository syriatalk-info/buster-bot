    ...
    for x in db.keys():
        for c in db[x].keys():
            if x!= source[3] and c == parameters:
                reply(type, source, u'Бот уже сидит в этой комнате с учетки '+x)
                return
    db[source[3]][parameters]={'nick':DEFAULT_NICK, 'status':'', 'show':''}
...
def hnd_leave(type, source, parameters):
    ...
    if not parameters:
        ...
        if not source[3] in db:
            ...
            return
        if not parameters in db[source[3]] and not parameters in GROUPCHATS:
            reply(type, source, u'Меня там нет!')
            return
        if parameters in db[source[3]]:
            del db[source[3]][parameters]
            write_file(file, str(db))
        if parameters in GROUPCHATS:
            del GROUPCHATS[parameters]
        leave(parameters, '', source[3])
