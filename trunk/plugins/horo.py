...
def hnd_horoscope(t, s, p):
    ...
    if not jid in HOROS.keys() and not p or p.lower()==u'все' or p and not p.isdigit():
        ...
        try: page = urllib.urlopen(url).read()
            ...
            return
        name = re.findall('<a href=\"/goroskop/\d{1}/\d{1}/\">(.*?)</a>',page)
        urls = re.findall('<a href=\"(/goroskop/\d{1}/\d{1}/)\">.*?</a>',page)
        d = {}
        n = 0
        rep = ''
...
register_command_handler(hnd_horoscope, 'гороскоп', ['все'], 0, 'Гороскоп', 'гороскоп', ['гороскоп'])
