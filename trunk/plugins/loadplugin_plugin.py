...
def svn_update_plugin(t, s, p):
  if not p:
    url = 'http://buster-bot.googlecode.com/svn/trunk/buster.py'
    ...
    if not re.findall('def (.*?)\(', string,  re.IGNORECASE):
      ...
      return
    write_file('buster.py',str(string))
    reply(t, s, u'Ядро обновлено!')
    return
  ss = [p]
  if p.count(' '):
    ss = p.split()
...
register_command_handler(svn_update_plugin, 'обновиплаг', ['все'], 99, 'Обновляет плаг с SVN.\nБез параметров обновит ядро.\nДля обновления до текущей ревизии возможно указывать команду используему в плагине, в остальных случаях - имя файла плагина целиком.', 'обновиплаг <plugin> <rev>', ['обновиплаг ibb_plugin.py','обновиплаг admin.py 55'])
