...
register_command_handler(hnd_banlastmuc, 'банласт', ['все'], 20, 'Бан последних пользователей', 'банласт', ['банласт'])                
...
register_command_handler(hnd_nnb_ban_set, 'бани нубов', ['все'], 20, 'Включает автобан пользователей ранее не бывших в чате, смотрит по базе жидов, ранг не важен', 'бани нубов', ['бани нубов'])
...
def hnd_nicks_inf(t, s, p):
        ...
        n = 0
        ij = False
        if len(p)>2 and p.count('@') and p[-3:][:1]=='.':
                ij = True
        for x in NICKS[s[1]]:
                ishere = False
                if x.count('@conference.'):
                        continue
                if p in NICKS[s[1]][x]['n'] or ij and p.lower()==x.lower():
