...
def hnd_turboping(t, s, p):
    ...
    def turboping_handler(x, jid, i, type, s):
        ...
        if x['type'] == 'result':
            ...
            if int(x['id'])==0:
                reply(type, s, u'Понг от '+x['from']+' '+str(round(t-i, 3))+'c.')
    ...
    if len(TURBO_PING[jid]['jid'])<3:
        if usr in TRB_BUF.keys():
            del TRB_BUF[usr]
            reply(t, s, u'Детальное тестирование невозможно, часть пакетов утеряна!')
        return
    try:
    ...
    rep = ('\n- '+str(rez)+u' секунд.\n' if rez>0 else u'\n - Понг реактивен! Результат ушел в минус!\n')
    ...
    rep += u'- Ботпинг '+str(round(bot, 2))+u'c. ;\n'
...
def tm_grab():
    i, rep = u'Time in world:\n', ''
    cities = {'New York':('40.7142700','-74.0059700'),'Moscow':('55.7522200','37.6155600')}
    try:
        for x in cities:
            c = urllib.urlopen('http://www.earthtools.org/timezone/'+cities[x][0]+'/'+cities[x][1]).read()
            rep += x+' - '+re.findall('<localtime>(.*?)</localtime>',c, re.DOTALL|re.IGNORECASE)[0]+'\n'
    except: pass
    return (i if rep.isspace() else i+rep)
