...
def join_user_ping(g, n, r, a):
    ...
    if a in ['participant','visitor']:
        if a==u'participant':
            room_access(g, 'role', 'visitor', 'nick', n)
        packet = IQ(JAB, 'get')
        packet.addElement('query', 'jabber:iq:version')
        packet.addCallback(user_ping_result, g, n)
        reactor.callFromThread(packet.send, jid)

def user_ping_result(g, n, x):
    if x['type'] == 'result':
        time.sleep(3)
        room_access(g, 'role', 'participant', 'nick', n)
...
def hnd_user_ping(t, s, p):
    ...
    if p=='1':
        ...
        return
    if p==u'лимит' and p.split() and p.split()[1].isdigit():
...
register_command_handler(hnd_user_ping, 'юзер_пинг', ['все'], 20, 'Пингует вошедшего юзера, и только после ответа клиента выдает голос. Команда может использоваться против примитивных спам-ботов. Для установки лимита на количество выдачи голосов за минут используем ключ команды <лимит> <число>', 'юзер_пинг <0|1|лимит>', ['юзер_пинг 1','юзер_пинг лимит 1'])
