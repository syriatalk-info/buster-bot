...
def hnd_turbo_info(t, s, p):
    if p.lower() in TURBO_PING.keys():
        p = p.lower()
        reply(t, s, 'PING JID: '+', '.join(TURBO_PING[p]['jid'])+'\nPING BOT: '+', '.join(TURBO_PING[p]['bot']))
    else:
        reply(t, s, u'Нету инфы на '+p[:20])
...
def hnd_turboping(t, s, p):
    ...
    res = (l-bot)-s2s
    if res < 0:
        reply(t, s, u'Что-то глюкнуло!'+('\nPING JID: '+', '.join(TURBO_PING[jid]['jid'])+'\nPING BOT: '+', '.join(TURBO_PING[jid]['bot']) if len(TURBO_PING[jid]['bot'])>1 and len(TURBO_PING[jid]['jid'])>1 else ''))
        return
...
register_command_handler(hnd_turbo_info, 'турбоинфа', ['все'], 0, 'Информация на JID из истории по турбопингу', 'турбоинфа жид', ['турбоинфа жопа.ру'])
