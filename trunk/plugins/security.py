...
def security_leave(g, n, r, c):
    if g in GROUPCHATS:
        if c=='303':
            security_join(g, n, None, None)
        ...
        if time.time()-SECURITY_BUF[g]['time']<SECURITY[g]['time']:
            ...
            if SECURITY_BUF[g]['n']==SECURITY[g]['n']:
                if time.time()-LIM_INFO_LAST>860:
                    msg(g, u'/me Достигнут лимит Гостей.. Следующий - в топку!')
                    LIM_INFO_LAST=time.time()
            if SECURITY_BUF[g]['n']>SECURITY[g]['n']:
                if not jid.split('@')[1] in SECURITY_BUF[g]['serv']:
                    ban(g, jid)
                    ban(g, jid.split('@')[1])
                    SECURITY_BUF[g]['serv'].append(jid.split('@')[1])
