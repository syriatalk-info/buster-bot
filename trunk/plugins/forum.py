...
FORUM_NICK_FILE = 'dynamic/forum_nick.txt'
db_file(FORUM_NICK_FILE, dict)
...
def hnd_forum(t, s, p, n=0):
        ...
        if not jid in USER_FORUM.keys() or not p:# or p==u'дальше':
                ...
                if n>10:
                        ...
                        FORUM_PLUS[jid]=rep2
                rep+=u'\nДля просмотра темы отправьте номер, напр. 1\nЧтобы создать тему используйте <f+ Название темы # Ваше сообщение> без кавычек'
        ...
        if jid in USER_FORUM.keys() and p in USER_FORUM[jid]:
                ...
                rep += u'> '+USER_FORUM[jid][p][0]+'\n'
                list = subject_last_activ(USER_FORUM[jid][p], all=1)
                ind = list[len(list)-1]
                rep+='> '+frmt(ind[2])+'|'+ind[0]+': '+ind[1]+'\n\n'
                list.remove(ind)
...
def hnd_make_forum(t, s, p):
    ...
    if not p or len(c)<2:
        reply(t, s, u'Введите параметры: f+ тема # сообщение. прим. f+ Привет # всем куку, идемте бухать')
        return
    sub = c[0]
    if len(sub)<3:
...
def hnd_forum_msg(t, s, p):
    ...
    #m=(jid, 'm')
    #if not m in FORUM_LIMIT.keys():
    #        FORUM_LIMIT[m]=time.time()
