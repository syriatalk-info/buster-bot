...
def hnd_inmuc(t, s, p):
    if not s[1] in GROUPCHATS:
        return
    reply(t, s, ', '.join([x for x in GROUPCHATS[s[1]].keys() if GROUPCHATS[s[1]][x]['ishere']]))

register_command_handler(hnd_inmuc, 'инмук', ['все','инфо'], 0, 'Показывает кто в конференции.', 'инмук', ['инмук'])
