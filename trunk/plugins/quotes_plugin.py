...
def hnd_jaba(type,source,parameters):
        try:
                req = urllib2.Request('http://jabber-quotes.ru/random')
                req.add_header = ('User-agent', 'Mozilla/5.0')
                r = urllib2.urlopen(req)
                text = r.read()
                ...
                reply(type,source,u'Jabber-quotes накрылся мендным тазом!')
...
def kill_me_quotes(type, source, parameters):
        try:
                ...
                if rep:
                        if len(rep)<2:
                                reply(type, source, u'Цитата с номером '+n+u'не найдена!Попробуйте еще!')
                                return
                        rep=rep[0]
                        try:
                                txt=decode(rep)
