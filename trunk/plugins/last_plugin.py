...
def idle_handler(t, s, p):
    ...
    packet.addCallback(idle_result_handler, t, s, p)
    reactor.callFromThread(packet.send, jid)

def idle_result_handler(t, s, p, x):
    if x['type']=='result':
        t=x.children[0].getAttribute('seconds')
        reply(t, s, p+u' работает уже '+timeElapsed(int(t)))
    else:
        reply(t, s, u'Не получилось')
...
def last_handler(t, s, p):
    ...
    packet = IQ(JAB, 'get')
    packet.addElement('query', 'jabber:iq:last')
    packet.addCallback(last_result_handler, t, s, p, i)
    reactor.callFromThread(packet.send, jid)
    ...
    if x['type']=='result':
        try: tim=x.children[0]['seconds']
            ...
            for c in x.elements():
                ...
                if tim:
                    break
        rep+=u'У юзера '+p+u' последняя активность '+timeElapsed(int(tim))+u' назад\n'
    else:
        rep+=u'Нет ответа на iq:last\n'
    if i:
        print 'i'
        rep+=u'Последнее сообщение было отправлено '+timeElapsed(time.time()-GROUPCHATS[s[1]][p]['idle'])
    reply(t, s, rep)
...
def hnd_last_all(t, s, p):
    ...
    try:
        ...
        if not die:
            die=u'место свободно'
        if not liv:
            liv=u'всех убило'
        reply(t, s, u'Список живых:\n'+liv+u'\nПокоятся с миром:\n'+die)
    except: pass
    del LAST_IQ_ALL[s[1]]
