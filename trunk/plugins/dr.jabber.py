...
def hnd_drjabber(t, s, p):
    ...
    rep +=u'Open registation (outcast server): '+str(len(DR_JABBER['open']))+u', new '+str([x for x in DR_JABBER['open'].keys() if not x in txt.get('open',[])])+'\n'
    rep +=u'Servers send error: '+str(len(DR_JABBER['close']))+u', new '+str([x for x in DR_JABBER['close'].keys() if not x in txt.get('close',[])])
    reply(t, s, rep)
