...
def hnd_uptime_top(t, s, p):
    ...
    global UPTTOPGET
    if UPTTOPGET == True:
        reply(t,s,u'Список обновляется, примерное время ожидания 20 минут')
        return
    d={}
    ...
    n=0
    err=False
    if time.time()-os.path.getmtime(TOPUPSERV_FILE)>86400*3 or not db or p.lower()==u'-o':
        UPTTOPGET = True
        list = drjabber_grab()
        list.extend(drjab_grab2())
        list.extend([x for x in drjab_getmuc_servlist() if not x in list])
        if p.lower()==u'-o':
            try: list=eval(read_file(DR_JABBER_FILE))['open']
            except: err=True
        if len(list)<3:
            err=True
        if err:
            reply(t, s, u'Если вы являетесь админом конференции попробуйте обновить список kомандой dr.jabber update,после чего повторите попытку!')
            return
...
register_command_handler(hnd_uptime_top, 'выбрать сервер', ['админ','все'], 0, 'Топ-30 ОТКРЫТЫХ jabber-серверов на основе времени работы. \nПо умолчанию работает фильтр серверов с одинаковым аптаймом(серверы яндекса и т.д.) ключ -f выведет результат без фильтров.\nКлюч -o покажет сервера с окрытой регой.\nКОманда работа в режиме бета-тестирования.', 'выбрать сервер', ['выбрать сервер'])
